...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
Mat<T, DimM, DimN>::Mat(Mat &&other) noexcept
    : m_data(std::move(other.m_data)) {}

template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
Mat<T, DimM, DimN> &Mat<T, DimM, DimN>::operator=(const Mat &other) {
  m_data = other.m_data;
  return *this;
}
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
Mat<T, DimM, DimN> &Mat<T, DimM, DimN>::operator=(Mat &&other) noexcept {
  m_data = std::move(other.m_data);
  return *this;
}
...
template <int SubDimM, int SubDimN>
  requires((SubDimM > 0) && (SubDimM < DimM) && (SubDimN > 0) &&
           (SubDimN < DimN))
Mat<T, SubDimM, SubDimN> Mat<T, DimM, DimN>::submatrix(int row, int col) const {
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
T *Mat<T, DimM, DimN>::data() {
  return m_data.data();
}

template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
const T *Mat<T, DimM, DimN>::data() const {
  return m_data.data();
}
  ...
  for (int i = 0; i < DimM; ++i) {
    for (int j = 0; j < DimP; ++j) {
      for (int k = 0; k < DimN; ++k) {
        result.m_data[i * DimP + j] +=
            m_data[i * DimN + k] * other.m_data[k * DimP + j];
      }
    }
  }
  return result;
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
Vec<T, DimM> Mat<T, DimM, DimN>::operator*(const Vec<T, DimN> &vec) const {
  Vec<T, DimM> result;
  for (int i = 0; i < DimM; ++i) {
    for (int j = 0; j < DimN; ++j) {
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
T &Mat<T, DimM, DimN>::get(int row, int col) {
  return m_data[col * DimN + row];
}

template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
T Mat<T, DimM, DimN>::get(int row, int col) const {
  return m_data[col * DimN + row];
}
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
Mat<T, DimM, DimN> &Mat<T, DimM, DimN>::operator*=(T scalar) {
  for (int i = 0; i < DimM * DimN; ++i) {
    m_data[i] *= scalar;
  }
  return *this;
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
bool Mat<T, DimM, DimN>::operator==(const Mat &other) const {
  for (int i = 0; i < DimM * DimN; ++i) {
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
T *Mat<T, DimM, DimN>::begin() {
  return m_data.data();
}

template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
const T *Mat<T, DimM, DimN>::begin() const {
  return m_data.data();
}

template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
T *Mat<T, DimM, DimN>::end() {
  return m_data.data() + DimM * DimN;
}

template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
const T *Mat<T, DimM, DimN>::end() const {
  return m_data.data() + DimM * DimN;
}

template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
Mat<T, DimN, DimM> Mat<T, DimM, DimN>::transpose() const {
...
T Mat<T, DimM, DimN>::minor(int row, int col) const {
  ...
  for (int i = 0; i < DimM - 1; ++i) {
    for (int j = 0; j < DimN - 1; ++j) {
      submatrix.get(i, j) =
          m_data[(i < row ? i : i + 1) * DimN + (j < col ? j : j + 1)];
    }
  }
...
Mat<T, IDimM, IDimN> Mat<T, DimM, DimN>::identity() {
...
template <Arithmetic T, int DimM, int DimN>
  requires((DimM > 0) && (DimN > 0))
template <int IDimM, int IDimN>
  requires(std::is_floating_point_v<T> && (IDimM == DimM) && (IDimN == DimN) &&
           (IDimM == 4))
Mat<T, IDimM, IDimN> Mat<T, DimM, DimN>::perspective(T fov, T aspect, T near,
                                                     T far) {
  Mat<T, IDimM, IDimN> result;
...
template <int IDimM, int IDimN>
  requires(std::is_floating_point_v<T> && (IDimM == DimM) && (IDimN == DimN) &&
           (IDimM == 4))
Mat<T, IDimM, IDimN> Mat<T, DimM, DimN>::orthographic(T left, T right, T bottom,
                                                      T top, T near, T far) {
  Mat<T, IDimM, IDimN> result;
...
Mat<T, DimM, DimN>::translation(const Vec<T, IDimM - 1> &translation) {
  static_assert(DimM == DimN && DimM >= 3,
                "Translation matrix must be square and at least 3x3");
...
template <int IDimM, int IDimN>
  requires(std::is_arithmetic_v<T> && (IDimM == DimM) && (IDimN == DimN))
Mat<T, IDimM, IDimN> Mat<T, DimM, DimN>::rotation(const Vec<T, 3> &rotation) {
  static_assert(DimM == DimN && DimM == 4,
                "Rotation matrix must be square and 4x4");
...
template <int IDimM, int IDimN>
  requires(std::is_arithmetic_v<T> && (IDimM == DimM) && (IDimN == DimN))
Mat<T, IDimM, IDimN> Mat<T, DimM, DimN>::scale(const Vec<T, 3> &scale) {
  static_assert(DimM == DimN && DimM == 4,
                "Scale matrix must be square and 4x4");
...
template <int IDimM, int IDimN>
  requires(std::is_arithmetic_v<T> && (IDimM == DimM) && (IDimN == DimN))
Mat<T, IDimM, IDimN> Mat<T, DimM, DimN>::look_at(const Vec<T, 3> &eye,
                                                 const Vec<T, 3> &center,
                                                 const Vec<T, 3> &up) {
  static_assert(DimM == DimN && DimM == 4,
                "Look-at matrix must be square and 4x4");
